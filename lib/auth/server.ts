...
export async function requireFleetUser(): Promise<DashboardUser> {
  const response = await requestBackend("/api/v1/user/auth/me", {
    cookieHeader: (await cookies()).toString(),
  })
  // `redirect()` applies Next.js' configured basePath. Passing an already
  // prefixed path would produce `/fleet/fleet/...`.
  if (response.status === 401) {
    const returnTo = safeReturnTo((await headers()).get("x-fleet-return-to"))
  ...
  if (!payload.ok || !payload.user.roles.includes("Fleet")) {
