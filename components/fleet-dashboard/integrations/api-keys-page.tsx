...
export function FleetApiKeysPage({ access }: { access?: BusinessAccess }) {
  ...
  async function createKey(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalizedName) return showToast({ type: "error", title: "Check API key", message: "Key name is required." })
    if (normalizedName.length > 100) return showToast({ type: "error", title: "Check API key", message: "Key name must be 100 characters or fewer." })
    if (!scopes.length) return showToast({ type: "error", title: "Check API key", message: "Select at least one API scope." })
    setSaving(true)
    setNotice("")
    ...
    setSaving(false)
    if (!response.ok || payload?.ok === false) { const message = payload?.message ?? "Unable to create API key"; setNotice(message); showToast({ type: "error", title: "Unable to create API key", message }); return }
    setSecret(payload.apiKey)
    setKeys((current) => [payload.key, ...current])
    setName("My website backend")
    showToast({ type: "success", title: "API key created", message: "Your new API key is ready to copy." })
  }
  ...
  async function revokeSelectedKey() {
    ...
    const payload = await response.json().catch(() => ({}))
    setSaving(false)
    if (!response.ok || payload?.ok === false) { const message = payload?.message ?? "Unable to revoke API key"; setNotice(message); showToast({ type: "error", title: "Unable to revoke API key", message }); return }
    setKeys((current) => current.map((item) => item.id === revokeKey.id ? payload.apiKey : item))
    setRevokeKey(null)
    showToast({ type: "success", title: "API key revoked", message: "The API key can no longer be used." })
  }
  ...
  async function copyApiKey() {
    try {
      ...
      showToast({ type: "error", title: "Copy failed", message: "Unable to copy the API key to the clipboard." })
    }
  }
  ...
  return <main className="space-y-6">
    <Card><CardHeader><div className="flex items-center gap-3"><span className="rounded-lg border border-primary/20 bg-primary/10 p-2 text-primary"><KeyRound className="h-5 w-5" /></span><div><CardTitle>Developer API Keys</CardTitle><CardDescription>Use server-side API keys to connect your own website, ERP, or backend to your Fleet data.</CardDescription></div></div></CardHeader><CardContent className="grid gap-3 md:grid-cols-3"><Info label="Account" value={access.businessAccount.name} /><Info label="Plan" value={access.businessAccount.plan.name} /><Info label="API tier" value={apiAccess?.apiTier ?? "Checking"} /></CardContent></Card>
    ...
    <Card><CardHeader><CardTitle className="text-base">Create API key</CardTitle><CardDescription>The full key is shown once. Store it in your server environment, never in frontend JavaScript.</CardDescription></CardHeader><CardContent><form onSubmit={createKey} className="space-y-4"><div className="space-y-2"><Label htmlFor="api-key-name">Key name *</Label><Input id="api-key-name" value={name} maxLength={100} onChange={(event) => setName(event.target.value)} disabled={!apiAccess?.allowed || saving} required /></div><div className="grid gap-3 md:grid-cols-2">{scopeOptions.map((scope) => <label key={scope.key} className="flex gap-3 rounded-lg border border-border bg-card p-3 text-sm"><Checkbox checked={selectedScopes.has(scope.key)} onCheckedChange={(checked) => toggleScope(scope.key, checked)} disabled={!apiAccess?.allowed || saving} /><span><span className="font-medium">{scope.label}</span><span className="block text-xs text-muted-foreground">{scope.key}</span></span></label>)}</div><Button type="submit" disabled={!apiAccess?.allowed || saving || !scopes.length}>{saving ? "Creating..." : "Create key"}</Button></form></CardContent></Card>
