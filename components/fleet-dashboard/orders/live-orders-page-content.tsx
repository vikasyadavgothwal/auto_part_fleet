...
export function LiveOrdersPageContent({ initialOrders, initialPagination, initialSummary }: {
  ...
  return <div className="min-h-screen bg-[#0A0A0A]"><div className="space-y-8">
    ...
    <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="max-h-[92vh] overflow-y-auto border-[#2A2A2A] bg-[#151515] text-white sm:max-w-3xl"><DialogHeader><DialogTitle>{selected?.publicId}</DialogTitle><DialogDescription>{selected?.source === "rfq" ? `${selected.rfq?.publicId}: ${selected.rfq?.projectName}` : "Direct supplier order"}</DialogDescription></DialogHeader>{selected ? <div className="space-y-5"><div className="grid gap-2 rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] p-4 text-sm sm:grid-cols-2"><p><span className="text-[#9CA3AF]">Supplier:</span> {supplierName(selected)}</p><p><span className="text-[#9CA3AF]">Total:</span> {money(selected.totalAmount)}</p><p><span className="text-[#9CA3AF]">Status:</span> {labelStatus(selected.status)}</p><p><span className="text-[#9CA3AF]">Payment:</span> <span className="capitalize text-emerald-400">{selected.paymentStatus}</span></p><p><span className="text-[#9CA3AF]">Created:</span> {new Date(selected.createdAt).toLocaleString("en-AE")}</p>{selected.expectedDeliveryAt ? <p><span className="text-[#9CA3AF]">Expected delivery:</span> {new Date(selected.expectedDeliveryAt).toLocaleDateString("en-AE", { day: "numeric", month: "short", year: "numeric" })}</p> : null}{selected.rfq ? <><p><span className="text-[#9CA3AF]">Delivery:</span> {selected.rfq.deliveryRequirement}</p><p><span className="text-[#9CA3AF]">Terms:</span> {selected.rfq.paymentTerms}</p><p><span className="text-[#9CA3AF]">VIN:</span> {selected.rfq.vehicleVin || "-"}</p></> : null}<div className="sm:col-span-2"><p className="text-[#9CA3AF]">Progress</p><div className="mt-2 h-2 rounded-full bg-[#2A2A2A]"><div className="h-full rounded-full bg-[#DC2626]" style={{ width: `${selected.deliveryProgress}%` }} /></div><p className="mt-1 text-xs text-[#9CA3AF]">{selected.deliveredItemCount} of {selected.totalItemCount} items delivered ({selected.deliveryProgress}%)</p></div></div><div className="rounded-lg border border-[#2A2A2A] bg-[#1A1A1A] p-4 text-sm"><h3 className="mb-2 font-semibold">Delivery Address</h3><p className="break-words text-[#9CA3AF]">{deliveryAddress(selected) || "No delivery address saved for this order."}</p></div><div><h3 className="mb-2 font-semibold">Items</h3>{selected.items.map((item) => <div key={item.id} className="flex justify-between gap-4 border-t border-[#2A2A2A] py-3 text-sm"><div><p>{item.partName}</p><p className="text-[#9CA3AF]">{item.partNumber || "No part number"} | Qty {item.quantity}</p><p className="text-[#9CA3AF]">Delivery: {deliveryLabel(item.deliveryOption)}{item.expectedDeliveryAt ? ` | Expected ${new Date(item.expectedDeliveryAt).toLocaleDateString("en-AE")}` : ""}</p>{item.deliveredAt ? <p className="text-emerald-400">Delivered {new Date(item.deliveredAt).toLocaleString("en-AE")}{item.proofRecipientName ? ` to ${item.proofRecipientName}` : ""}</p> : null}{item.proofOfDeliveryNote ? <p className="text-[#9CA3AF]">{item.proofOfDeliveryNote}</p> : null}</div><div className="text-right"><p>{item.lineTotal === null ? "Included in quote" : money(item.lineTotal)}</p>{item.proofOfDeliveryUrl ? <a href={appPath(`/api/orders/${encodeURIComponent(selected.id)}/proof?itemId=${encodeURIComponent(item.id)}`)} target="_blank" rel="noreferrer" className="mt-1 inline-block text-xs text-[#DC2626] hover:underline">View proof</a> : null}</div></div>)}</div></div> : null}</DialogContent></Dialog>
  </div>
}
