...
export function FleetFeatureAccessPage({
  access,
  area,
  ...
  if (area === "add-ons") return <main className="space-y-5 sm:space-y-6">
    <Card className="border-border/80 bg-card shadow-sm">
      <CardHeader className="gap-4 p-4 sm:flex sm:flex-row sm:items-start sm:justify-between sm:p-6">
        <div>
          <p className="text-sm font-medium text-primary">Fleet add-ons</p>
        ...
        <Badge variant="outline" className="w-fit border-primary/30 bg-primary/10 px-3 py-1 text-primary">{access?.businessAccount.plan.name ?? "Plan unavailable"}</Badge>
      </CardHeader>
    </Card>
    {addOns.length ? <Card className="border-border/80 bg-card shadow-sm"><CardHeader className="p-4 sm:p-6"><CardTitle className="text-base">Your add-ons</CardTitle><CardDescription>Current and recent add-on requests.</CardDescription></CardHeader><CardContent className="grid gap-3 p-4 pt-0 sm:grid-cols-2 sm:p-6 sm:pt-0 lg:grid-cols-3">{addOns.map((item) => <div key={item.id} className="rounded-lg border border-border bg-background/70 p-4"><div className="grid gap-3 sm:flex sm:items-start sm:justify-between"><p className="font-medium text-foreground">{item.label}</p><Badge variant="outline" className={addOnStatusClass(item)}>{addOnStatusLabel(item)}</Badge></div><p className="mt-3 text-sm font-semibold text-foreground">{formatMoney(item.priceAmount, item.priceCurrency)}</p><p className="mt-1 text-xs text-muted-foreground">{formatValidUntil(item.validUntil)}</p></div>)}</CardContent></Card> : null}
    {addOnGroups.map((section) => <section key={section.title} className="space-y-4"><div><h2 className="text-lg font-semibold text-foreground">{section.title}</h2><p className="mt-1 max-w-3xl text-sm text-muted-foreground">{section.description}</p></div><Card className="border-border/80 bg-card shadow-sm"><CardContent className="p-0"><div className="overflow-x-auto"><Table className="min-w-[820px]"><TableHeader><TableRow><TableHead>Add-on</TableHead><TableHead>Usage</TableHead><TableHead>Extra units <span className="text-destructive">*</span></TableHead><TableHead>Price</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>{section.limits.map((item) => { const rawExtraUnits = limitTargets[item.metric] ?? String(item.suggestedExtraUnits ?? 5); const extraUnits = Number(rawExtraUnits); const featureKey = Number.isInteger(extraUnits) ? limitFeatureKey(item.metric, extraUnits) : item.key; const request = addOns.find((row) => row.featureKey === featureKey); const active = isAddOnCurrentlyActive(request); const waiting = request?.status === "Requested"; const currentLimit = item.currentLimit ?? 0; const addedCapacity = limitExtraUnits(extraUnits); const newTotalLimit = currentLimit + addedCapacity; const invalid = !Number.isInteger(extraUnits) || extraUnits < 1; const price = limitPriceSummary(item, addedCapacity); return <TableRow key={item.metric}><TableCell className="min-w-64"><p className="font-medium text-foreground">{item.label}</p><p className="mt-1 text-xs text-muted-foreground">Increase this limit for your fleet account.</p></TableCell><TableCell className="text-sm"><p className="text-muted-foreground">Current: <span className="font-medium text-foreground">{item.currentUsage}/{item.currentLimit ?? "Unlimited"}</span></p><p className="mt-1 text-muted-foreground">After: <span className="font-medium text-foreground">{Number.isInteger(extraUnits) ? newTotalLimit : "Enter units"}</span></p></TableCell><TableCell><input type="number" inputMode="numeric" min={1} step={1} className="h-9 w-24 rounded-md border border-border bg-background px-3 text-sm text-foreground outline-none focus:border-primary" value={rawExtraUnits} onChange={(event) => setLimitTargets((targets) => ({ ...targets, [item.metric]: event.target.value.replace(/\D/g, "").slice(0, 6) }))} /></TableCell><TableCell className="min-w-48"><p className="font-semibold text-foreground">{price.total}</p><p className="mt-1 text-xs text-muted-foreground">{price.quantity} · {price.unit}</p><p className="mt-1 text-xs text-muted-foreground">{formatValidity(item.validityDays)}</p></TableCell><TableCell>{request ? <Badge variant="outline" className={addOnStatusClass(request)}>{addOnStatusLabel(request)}</Badge> : <Badge variant="outline">Available</Badge>}</TableCell><TableCell className="text-right"><Button size="sm" disabled={pendingKey === featureKey || active || waiting || invalid} onClick={() => requestAddOn(featureKey, `Add ${addedCapacity} extra units. New total limit after add-on: ${newTotalLimit}.`)}>{pendingKey === featureKey ? "Adding..." : active ? "Already added" : waiting ? "Payment pending" : request?.status === "Rejected" ? "Add again" : "Add limit"}</Button></TableCell></TableRow> })}{section.features.map((feature) => { const request = addOns.find((item) => item.featureKey === feature.key); const active = isAddOnCurrentlyActive(request); const waiting = request?.status === "Requested"; return <TableRow key={feature.key}><TableCell className="min-w-64"><p className="font-medium text-foreground">{feature.label}</p><p className="mt-1 text-xs text-muted-foreground">Unlock this feature without changing your current plan.</p></TableCell><TableCell className="text-sm text-muted-foreground">Feature add-on</TableCell><TableCell className="text-muted-foreground">-</TableCell><TableCell className="min-w-48"><p className="font-semibold text-foreground">{formatMoney(feature.priceAmount, feature.priceCurrency)}</p><p className="mt-1 text-xs text-muted-foreground">{formatValidity(feature.validityDays)}</p></TableCell><TableCell>{request ? <Badge variant="outline" className={addOnStatusClass(request)}>{addOnStatusLabel(request)}</Badge> : <Badge variant="outline">Available</Badge>}</TableCell><TableCell className="text-right"><Button size="sm" disabled={pendingKey === feature.key || active || waiting} onClick={() => requestAddOn(feature.key)}>{pendingKey === feature.key ? "Adding..." : active ? "Already added" : waiting ? "Payment pending" : request?.status === "Rejected" ? "Add again" : "Add add-on"}</Button></TableCell></TableRow> })}</TableBody></Table></div></CardContent></Card></section>)}
    ...
    <PaymentHistoryTable
      ...
      description="All paid Common and Fleet add-ons for this fleet account."
      showDuration
      showExpiry
    ...
    <Dialog open={Boolean(pendingAddOnConfirmation)} onOpenChange={(open) => { if (!open) setPendingAddOnConfirmation(null) }}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>This add-on will be enabled for your fleet account.</DialogDescription>
        </DialogHeader>
  ...
  async function createTicket(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (normalizedSubject.length < 3 || normalizedSubject.length > 150) return showToast({ type: "error", title: "Check support ticket", message: "Subject must be between 3 and 150 characters." })
    if (normalizedMessage.length < 10 || normalizedMessage.length > 2000) return showToast({ type: "error", title: "Check support ticket", message: "Message must be between 10 and 2,000 characters." })
  ...
  return <main className="space-y-6">
    <section className="rounded-xl border border-border bg-card p-6 shadow-sm">
      <div className="grid gap-5">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-foreground">How can we help your fleet?</h1>
    ...
    <Card className="border-border/80 bg-card shadow-sm">
      <CardHeader>
        ...
        <CardDescription>Step-by-step answers for common fleet dashboard questions.</CardDescription>
      </CardHeader>
      <CardContent>
    ...
    <Card className="border-border/80 bg-card shadow-sm">
      <CardHeader>
        ...
        <CardDescription>Short walkthroughs for the fleet dashboard workflow.</CardDescription>
      </CardHeader>
      <CardContent>
    ...
    <Dialog open={isTicketDialogOpen} onOpenChange={setIsTicketDialogOpen}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Send the issue to Admin support for this fleet account.</DialogDescription>
        </DialogHeader>
