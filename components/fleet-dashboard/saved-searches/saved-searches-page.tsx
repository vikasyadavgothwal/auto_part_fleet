...
const scopeOptions = [
  { value: "fleet.rfqs", label: "RFQs", route: appRoutes.rfqs },
  { value: "fleet.orders", label: "Orders", route: appRoutes.orders },
  { value: "fleet.vehicles", label: "Vehicles", route: appRoutes.vehicles },
] as const
...
export function FleetSavedSearchesPage({
  access,
  ...
  const limitText = formatLimit(
    access?.businessAccount?.usage?.savedSearches,
    access?.businessAccount?.limits?.savedSearches,
  )

  const savedSearches = useMemo(() => initialSavedSearches, [initialSavedSearches])
  ...
  return (
    <div className="space-y-6 text-white">
      <div>
        ...
        <p className="mt-1 text-sm text-[#9CA3AF]">Save repeat fleet filters and re-run them from one place.</p>
        <p className="mt-2 text-xs text-muted-foreground">Plan usage: {limitText} saved searches</p>
      </div>
      ...
      <section className="grid gap-3">
        {savedSearches.length ? savedSearches.map((item) => {
          ...
          return (
            <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#1f2937] bg-[#111827] p-4">
              <div>
                <p className="font-semibold">{item.name}</p>
                <p className="text-sm text-[#9CA3AF]">{option.label} · {String(item.query?.q ?? "No keyword")}</p>
              </div>
          ...
          <p className="rounded-lg border border-[#1f2937] bg-[#111827] p-4 text-sm text-[#9CA3AF]">No saved searches yet.</p>
        )}
      </section>
    </div>
  )
