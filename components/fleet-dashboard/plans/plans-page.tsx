...
type BusinessPlan = {
  ...
  limits: { staff: number | null; roles: number | null; vehicles: number | null; rfqs: number | null; orders: number | null }
...
type BusinessAccess = {
  businessAccount: { id: string; type?: string; plan: BusinessPlan; usage?: { staff?: number; vehicles?: number; rfqs?: number; orders?: number } }
...
const isFleetAccess = (item: BusinessAccess) =>
...
async function readPlanData() {
  ...
  const access = accessPayload.access?.find(isFleetAccess)
  ...
  return { access, plans: (plansPayload.plans ?? []).filter((plan) => plan.accountType === "Fleet"), transactions: access?.paymentTransactions ?? transactionsPayload.transactions ?? [] }
}
...
export async function PlansPage({ paymentStatus = "none" }: { paymentStatus?: PaymentReturnStatus }) {
  ...
  const usageCards = currentPlan ? [{ label: "Staff", value: toNumber(usage?.staff), limit: currentPlan.limits.staff }, { label: "Vehicles", value: toNumber(usage?.vehicles), limit: currentPlan.limits.vehicles }, { label: "RFQs", value: toNumber(usage?.rfqs), limit: currentPlan.limits.rfqs }, { label: "Orders", value: toNumber(usage?.orders), limit: currentPlan.limits.orders }] : []
  ...
  return <div className="space-y-6">
    ...
    {currentPlan?.code !== "Enterprise" ? <Card>
      <CardHeader>
        ...
        <CardDescription>Permissions enabled by admin for this Fleet account, with expiry and renewal dates.</CardDescription>
      </CardHeader>
      <CardContent>
        {activeAddOns.length ? (
    ...
    <section className="grid gap-4 lg:grid-cols-3">{plans.map((plan) => { const isCurrent = isSamePlan(plan, currentPlan); const isDowngrade = Boolean(currentPlan && planRank[plan.code] < planRank[currentPlan.code]); return <div key={plan.id} className={`rounded-lg border bg-card p-5 shadow-sm ${isCurrent ? "border-primary" : "border-border"}`}><div className="flex items-start justify-between gap-3"><div><h2 className="text-lg font-semibold">{plan.name}</h2><p className="mt-1 text-sm text-muted-foreground">{plan.description}</p></div>{isCurrent ? <span className="rounded-full bg-primary px-2 py-1 text-xs text-primary-foreground">Active</span> : null}</div>{plan.code !== "Free" ? <div className="mt-4"><BillingPrice code={plan.code} currency={plan.price.currency} monthlyAmount={plan.price.amount} yearlyAmount={plan.price.yearlyAmount} /></div> : null}<dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-muted-foreground">Staff</dt><dd>{limitText(plan.limits.staff)}</dd></div><div><dt className="text-muted-foreground">Roles</dt><dd>{limitText(plan.limits.roles)}</dd></div><div><dt className="text-muted-foreground">Vehicles</dt><dd>{limitText(plan.limits.vehicles)}</dd></div><div><dt className="text-muted-foreground">RFQs</dt><dd>{limitText(plan.limits.rfqs)}</dd></div><div><dt className="text-muted-foreground">Orders</dt><dd>{limitText(plan.limits.orders)}</dd></div><div><dt className="text-muted-foreground">Reports</dt><dd>{reportText(plan)}</dd></div><div><dt className="text-muted-foreground">Login security</dt><dd>{securityText(plan)}</dd></div><div><dt className="text-muted-foreground">Support</dt><dd>{supportText(plan)}</dd></div><div><dt className="text-muted-foreground">API access</dt><dd>{apiText(plan)}</dd></div></dl>{isCurrent ? <Button className="mt-5 w-full" variant="secondary" disabled>Active Plan</Button> : currentPlan && access ? <ChangePlanButton businessAccountId={access.businessAccount.id} currentPlanName={currentPlan.name} planId={plan.id} planName={plan.name} currency={plan.price.currency} monthlyAmount={plan.price.amount} yearlyAmount={plan.price.yearlyAmount} isDowngrade={isDowngrade} actionLabel={isDowngrade ? "Downgrade Plan" : "Upgrade Plan"} /> : <Button className="mt-5 w-full" variant="secondary" disabled>Plan unavailable</Button>}</div> })}</section>
    <PaymentHistoryTable accountLabel="Fleet" transactions={transactions} />
  </div>
}
