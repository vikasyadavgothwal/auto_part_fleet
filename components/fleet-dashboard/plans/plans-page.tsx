...
async function readPlanData() {
  ...
  return {
    access: accessPayload.access?.find((item) => item.businessAccount.plan.accountType === "Fleet"),
    plans: (plansPayload.plans ?? []).filter((plan) => plan.accountType === "Fleet"),
  }
}
...
export async function PlansPage() {
  ...
  const usageCards = currentPlan ? [
    { label: "Vehicles", value: toNumber(usage?.vehicles), limit: currentPlan.limits.vehicles },
    { label: "RFQs", value: toNumber(usage?.rfqs), limit: currentPlan.limits.rfqs },
    { label: "Orders", value: toNumber(usage?.orders), limit: currentPlan.limits.orders },
          ...
          return (
            <div key={plan.id} className={`rounded-lg border bg-card p-5 shadow-sm ${isCurrent ? "border-primary" : "border-border"}`}>
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt className="text-muted-foreground">Vehicles</dt><dd>{limitText(plan.limits.vehicles)}</dd></div>
